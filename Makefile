## Makefile
## Summary: Cross-compilation builder for chml.
##
## Author:  KaisarCode
...
NATIVE_EXE_EXT :=
NATIVE_SHARED_NAME := libchml.so
NATIVE_IMPORT_LIBRARY :=

ifeq ($(NATIVE_PLATFORM),windows)
NATIVE_EXE_EXT := .exe
NATIVE_SHARED_NAME := libchml.dll
NATIVE_IMPORT_LIBRARY := -DCHML_TEST_IMPORT_LIBRARY=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/libchml.dll.a
endif
...
define linux_target
	...
	@if [ ! -f $(BUILD_DIR)/$(subst /,-,$(1))-linux/CMakeCache.txt ]; then \
		cmake -S . -B $(BUILD_DIR)/$(subst /,-,$(1))-linux \
			...
			-DCHML_BUILD_VERSION=$(BUILD_VERSION) \
			-DCMAKE_C_COMPILER=$(2) \
	...
	@cp $(BUILD_DIR)/$(subst /,-,$(1))-linux/out/chml $(BIN_DIR)/$(1)/linux/chml
	@echo "OK $(1)/linux"
endef
...
define windows_target
	...
	$(call cmake_build,$(BUILD_DIR)/$(1)-windows,cmake -S . -B $(BUILD_DIR)/$(1)-windows -DCMAKE_BUILD_TYPE=Release -DCMAKE_SYSTEM_NAME=Windows -DCHML_BUILD_VERSION=$$ver -DCMAKE_C_COMPILER=$(2) -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=$(CURDIR)/$(BUILD_DIR)/$(1)-windows/out -DCMAKE_ARCHIVE_OUTPUT_DIRECTORY=$(CURDIR)/$(BIN_DIR)/$(1)/windows -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=$(CURDIR)/$(BIN_DIR)/$(1)/windows -G Ninja -Wno-dev > /dev/null)
	@cp $(BUILD_DIR)/$(1)-windows/out/chml.exe $(BIN_DIR)/$(1)/windows/chml.exe
	@cp $(BUILD_DIR)/$(1)-windows/out/libchml.dll $(BIN_DIR)/$(1)/windows/libchml.dll
	@echo "OK $(1)/windows"
endef
...
test:
	@if [ -n "$(filter wine,$(MAKECMDGOALS))" ]; then \
		...
		if [ ! -f $(BIN_DIR)/x86_64/windows/libchml.dll ] || [ ! -f $(BIN_DIR)/x86_64/windows/libchml.dll.a ] || [ ! -f $(BIN_DIR)/x86_64/windows/chml.exe ]; then \
		...
		if [ ! -f $(BUILD_DIR)/test-wine/CMakeCache.txt ]; then \
			cmake -S . -B $(BUILD_DIR)/test-wine \
				...
				-DCHML_BUILD_TESTS=ON \
				-DCHML_TEST_SHARED_LIBRARY=$(CURDIR)/$(BIN_DIR)/x86_64/windows/libchml.dll \
				-DCHML_TEST_IMPORT_LIBRARY=$(CURDIR)/$(BIN_DIR)/x86_64/windows/libchml.dll.a \
				-DCHML_TEST_CLI=$(CURDIR)/$(BIN_DIR)/x86_64/windows/chml.exe \
				-DCMAKE_CROSSCOMPILING_EMULATOR=$(WINE) \
				-G Ninja -Wno-dev > /dev/null; \
		fi; \
		cmake --build $(BUILD_DIR)/test-wine --target chml_contract_test || exit 1; \
		...
		if [ ! -f $(BIN_DIR)/$(NATIVE_TARGET)/$(NATIVE_SHARED_NAME) ] || [ ! -f $(BIN_DIR)/$(NATIVE_TARGET)/chml$(NATIVE_EXE_EXT) ]; then \
		...
		if [ ! -f $(BUILD_DIR)/test/CMakeCache.txt ]; then \
			cmake -S . -B $(BUILD_DIR)/test \
				-DCMAKE_BUILD_TYPE=Release \
				-DCHML_BUILD_TESTS=ON \
				-DCHML_TEST_SHARED_LIBRARY=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/$(NATIVE_SHARED_NAME) \
				-DCHML_TEST_CLI=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/chml$(NATIVE_EXE_EXT) \
				$(NATIVE_IMPORT_LIBRARY) \
				-G Ninja -Wno-dev > /dev/null; \
		fi; \
		cmake --build $(BUILD_DIR)/test --target chml_contract_test || exit 1; \
